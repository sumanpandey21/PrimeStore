...
const MobileMenu = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className={`lg:hidden fixed inset-0 z-20 transition-all duration-300 ${
    ...
    >
      ...
      {/* Slide-in Menu */}
      <div
        className={`fixed top-0 left-0 bg-white z-50 flex flex-col w-64 sm:w-80 h-full shadow-lg transform transition-transform duration-300 overflow-y-auto ${
        ...
        <div className="flex items-center p-3.5 border-b border-gray-200 sticky top-0 bg-white z-10">
          ...
          >
            <Image
              ...
              height={48}
            />
            <div className="hidden sm:block text-xl font-bold mt-1 ml-[-5]">
              PrimeStore
            </div>
          </Link>
        </div>
          ...
          {navItems.map((item, index) => {
            ...
            return (
              <Link
                ...
                href={item.path}
                onClick={onClose}
                className={`w-full rounded-md px-3 py-2 transition-colors duration-200 hover:bg-gray-100 ${
